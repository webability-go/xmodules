...
func Synchronize(ctx *context.Context, prefix string) ([]string, error) {
	...
	if vc == "" {
		messages = append(messages, "xmodules/context need to be installed before installing xmodules/suggestions.")
		return messages, nil
	}
	...
	err := context.AddModule(ctx, MODULEID, "Suggestions", VERSION)
	if err == nil {
