...
func Run(ctx *context.Context, template *xcore.XTemplate, xlanguage *xcore.XLanguage, e interface{}) interface{} {
	...
	params := &xcore.XDataset{
		"FORMLANGUAGE": createXMLMask("formlanguage", mode, ctx),
		"KEY":          key,
		"#":            language,
	}
...
func build(mask *xdommask.Mask, ctx *context.Context) error {
	...
	if kl_language == nil {
		return errors.New(tools.LogMessage(ctx.LoggerError, language, "errortable", "build", "kl_language"))
	}

	//	mode := ctx.Request.Form.Get("mode")
	mask.Table = kl_language
	...
	f10.MaxLength = 5
	f10.Size = "400"
	f10.Format = "^[a-z][a-z0-9-_]{1,4}$"
	f10.FormatJS = "^[a-z][a-z0-9-_]{1,4}$"
	f10.DefaultValue = ""
	mask.AddField(f10)
	...
	// default language
	f111 := xdommask.NewLOVField("default")
	f111.Title = "##default.title##"
	f111.InRecord = true
	f111.HelpDescription = "##default.help.description##"
	...
	f111.StatusNotNull = "##default.status.notnull##"
	...
	f21.CheckJS = "n = WA.$N('translationadmin/language/editor|" + key + "|languagetoprune'); n.domNodeField.disabled = (value==1); n.checkAll();"
	f21.URLVariable = "prune"
	mask.AddField(f21)

	// language
	f22 := xdommask.NewLOVField("languagetoprune")
	f22.Title = "##languagetoprune.title##"
	f22.HelpDescription = "##languagetoprune.help.description##"
	f22.NotNullModes = xdommask.DELETE
	f22.AuthModes = xdommask.DELETE
	f22.HelpModes = xdommask.DELETE
	f22.Table = kl_language
	...
	f22.URLVariable = "languagetoprune"
	mask.AddField(f22)
...
func predelete(m *xdommask.Mask, ctx *context.Context, key interface{}, oldrec *xdominion.XRecord, rec *xdominion.XRecord) error {
	...
	if prune == "1" {
		...
		// prune children
		group, _ := rec.GetString("languagetoprune")
...
func Formlanguage(ctx *context.Context, template *xcore.XTemplate, language *xcore.XLanguage, e interface{}) interface{} {
	...
	mask, _ := createMask("formlanguage", ctx)
	data, _ := mask.Run(ctx)
	return data
}
