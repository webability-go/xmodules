...
const (
	TRANSLATIONSOURCE_FILTER = "TRANSLATIONSOURCE_FILTER"
)

func Run(ctx *context.Context, template *xcore.XTemplate, language *xcore.XLanguage, e interface{}) interface{} {

	ok := security.Verify(ctx, security.USER, assets.ACCESSSOURCE)
	if !ok {
		return ""
	}
	...
	filter := entries.GetUserParam(ds, userkey, TRANSLATIONSOURCE_FILTER)
...
func Sourcedata(ctx *context.Context, template *xcore.XTemplate, language *xcore.XLanguage, e interface{}) interface{} {
	...
	search := userentries.GetUserParam(ds, userkey, TRANSLATIONSOURCE_FILTER)
	...
	total := entries.GetSourcesCount(ds, &filter)
	...
	for _, rg := range dataarray {
		...
		sources := entries.GetSourcesList(ds, &filter, &order, rg[1]-rg[0]+1, rg[0])
		if sources != nil {
			for _, recsource := range *sources {
				key, _ := recsource.GetString("key")
				name, _ := recsource.GetString("name")
				status, _ := recsource.GetString("config")

				update := ""
				delete := ""
				if key[0] != '_' {
					update = "<button class=\"button update\" onclick=\"translationadmin_source_go('" + key + "', 2);\">" + language.Get("button.edit") + "</button>"
					delete = "<button class=\"button delete\" onclick=\"translationadmin_source_go('" + key + "', 3);\">" + language.Get("button.delete") + "</button>"
				}
				// quantity of translations into the source
				...
				rec := &xdominion.XRecord{
					...
					"estatus":  sstatus,
					"quantity": quantity,
					"commands": "<button class=\"button view\" onclick=\"translationadmin_source_go('" + key + "', 4);\">" + language.Get("button.view") + "</button> " + update + " " + delete,
				}
...
func Filter(ctx *context.Context, template *xcore.XTemplate, language *xcore.XLanguage, e interface{}) interface{} {
	...
	userentries.SetUserParam(ds, userkey, TRANSLATIONSOURCE_FILTER, filter)
