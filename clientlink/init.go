...
func init() {
	m := &context.Module{
		ID:           MODULEID,
		Version:      VERSION,
		Languages:    map[language.Tag]string{language.English: "Client Link", language.Spanish: "Liga clientes", language.French: "Lier les clients"},
...
func Synchronize(ctx *context.Context, prefix string) ([]string, error) {
	...
	if vc == "" {
		messages = append(messages, "xmodules/context need to be installed before installing xmodules/clientlink.")
		return messages, nil
	}
	...
	err := context.AddModule(ctx, MODULEID, "Client Link", VERSION)
	if err == nil {
