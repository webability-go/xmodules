...
func init() {
	m := &base.Module{
		ID:           MODULEID,
		Version:      VERSION,
		Languages:    map[language.Tag]string{language.English: "Users linked from administration", language.Spanish: "Usuarios ligados de administración", language.French: "Utilisateurs ligués de l'administration"},
		Needs:        []string{"base"},
		FSetup:       Setup,
...
func Synchronize(ds applications.Datasource, prefix string) ([]string, error) {
	...
	if vc == "" {
		messages = append(messages, "xmodules/base need to be installed before installing xmodules/userlink.")
		return messages, nil
	}
	...
	err := base.AddModule(ctx, MODULEID, "Administration users linked", VERSION)
	if err == nil {
