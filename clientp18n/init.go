...
func Synchronize(ctx *context.Context, prefix string) ([]string, error) {
	...
	if vc == "" {
		messages = append(messages, "xmodules/context need to be installed before installing xmodules/client18n.")
		return messages, nil
	}
	...
	err := context.AddModule(ctx, MODULEID, "Cleint Personalization", VERSION)
	if err == nil {
