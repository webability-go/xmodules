package log

import (
	//"fmt"

	//"github.com/webability-go/xcore/v2"
	"github.com/webability-go/xdominion"

	"github.com/webability-go/xamboo/applications"

	//"github.com/webability-go/xmodules/adminmenu"
	"github.com/webability-go/xmodules/base"
	//"github.com/webability-go/xmodules/tools"
	//"github.com/webability-go/xmodules/user"
	//"github.com/webability-go/xmodules/log/assets"
	//userassets "github.com/webability-go/xmodules/user/assets"
)

// Order to load/synchronize tables:
var moduletablesorder = []string{
	"log_log",
}

// map[string] does not respect order
var moduletables = map[string]func() *xdominion.XTable{
	"log_log": log_log,
}

func linkTables(ds applications.Datasource) {

	langs := ds.GetLanguages()

	for _, tbl := range moduletablesorder {
		table := moduletables[tbl]()
		table.SetBase(ds.GetDatabase())
		ds.SetTable(tbl, table)
		table.SetLanguage(langs[0])
	}
}

func synchroTables(ds applications.Datasource, oldversion string) (error, []string) {

	result := []string{}

	for _, tbl := range moduletablesorder {

		err, res := base.SynchroTable(ds, tbl)
		result = append(result, res...)
		if err != nil {
			return err, result
		}
	}

	if oldversion < "0.0.1" {
		// do things
	}

	return nil, result
}

/*func install(ds applications.Datasource) (error, []string) {

	result := []string{}

	// Accesses
	err := user.AddAccessGroup(ds, &userassets.AccessGroup{
		Key:         assets.ACCESSGROUP,
		Name:        tools.Message(messages, "loggroup.name"),
		Description: tools.Message(messages, "loggroup.description"),
	})
	if err != nil {
		result = append(result, err.Error())
		return err, result
	}

	err = user.AddAccess(ds, &userassets.Access{
		Key:         assets.ACCESS,
		Name:        tools.Message(messages, "log.name"),
		Group:       assets.ACCESSGROUP,
		Description: tools.Message(messages, "log.description"),
	})
	if err != nil {
		result = append(result, err.Error())
		return err, result
	}

	mainoption := xdominion.XRecord{
		"key":          "_logadmin",
		"group":        "_admin",
		"father":       nil,
		"name":         tools.Message(messages, "logfolder.name"),
		"access":       assets.ACCESS,
		"icon1":        "folder.png",
		"call1":        "openclose",
		"description1": tools.Message(messages, "logfolder.description"),
	}
	err = adminmenu.AddOption(ds, &mainoption)
	if err != nil {
		result = append(result, err.Error())
		return err, result
	}

	option := xdominion.XRecord{
		"key":          "_logadmin_option",
		"group":        "_admin",
		"father":       "_logadmin",
		"name":         tools.Message(messages, "logoption.name"),
		"access":       assets.ACCESS,
		"icon1":        "option.png",
		"call1":        "logadmin|single",
		"description1": tools.Message(messages, "logoption.description"),
	}
	err = adminmenu.AddOption(ds, &option)
	if err != nil {
		result = append(result, err.Error())
		return err, result
	}

	return nil, []string{
		fmt.Sprint("logadmin options added"),
	}
}*/

func upgrade(ds applications.Datasource, oldversion string) (error, []string) {

	if oldversion < "0.0.1" {
		// do things
		//return install(ds)
		return nil, []string{}
	}
	//return install(ds)

	return nil, []string{}
}
